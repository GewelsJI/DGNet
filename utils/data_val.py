...
class CamObjDataset(data.Dataset):
    def __init__(self, image_root, gt_root, grad_root, trainsize):
        ...
        # get filenames
        if DEBUG:
            print('>>> dataloader (debug mode)')
            self.images = [image_root + f for f in os.listdir(image_root) if f.endswith('.jpg')][:200]
            self.gts = [gt_root + f for f in os.listdir(gt_root) if f.endswith('.jpg') or f.endswith('.png')][:200]
            self.grads = [grad_root + f for f in os.listdir(grad_root) if f.endswith('.jpg') or f.endswith('.png')][:200]
        else:
            print('>>> dataloader (train/val mode)')
...
# test dataset and loader
class test_dataset_grad:
    def __init__(self, image_root, gt_root, grad_root, testsize):
        self.testsize = testsize
        self.images = [image_root + f for f in os.listdir(image_root) if f.endswith('.jpg') or f.endswith('.png')]
        self.gts = [gt_root + f for f in os.listdir(gt_root) if f.endswith('.tif') or f.endswith('.png')]
        self.grads = [grad_root + f for f in os.listdir(grad_root) if f.endswith('.tif') or f.endswith('.png')]
    ...
    def load_data(self):
        ...
        grad = self.binary_loader(self.grads[self.index])
        grad = self.grad_transform(grad).unsqueeze(0)
        ...
        return image, gt, grad, name, np.array(image_for_post)
