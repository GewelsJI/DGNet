...
from jittor_lib.lib.DGNet import DGNet as jt_Network
from pytorch_lib.lib.DGNet import DGNet as py_Network
...
pytorch_model = py_Network(channel=64, arc='B4', M=[8, 8, 8], N=[4, 8, 16]).cuda()
...
for i in range(turns):
    ...
    jittor_result[0][0].sync() # sync是把计算图发送到计算设备上
jt.sync_all(True)
end = time.time()
