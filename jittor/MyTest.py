...
def evaluator(snap_path, val_root, gpu_id, trainsize=352, if_save_map=False):
    ...
    with jt.no_grad():
        for i in range(val_loader.size):
            ...
            image = image.cuda()
            output = model(image)
            output = nn.upsample(output[0], size=gt.shape, mode='bilinear')
            output = output.sigmoid().data.cpu().numpy().squeeze()
            output = ((output - output.min()) / ((output.max() - output.min()) + 1e-08))
            if if_save_map:
    ...
    tb.field_names = ['Method', 'Dataset', 'Smeasure', 'wFmeasure', 'MAE', 'adpEm', 'meanEm', 'maxEm', 'adpFm',
                      'meanFm', 'maxFm']
    tb.add_row([snap_path.split('/')[(- 2)], val_root.split('/')[(- 2)], sm.round(4), wfm.round(4), mae.round(4),
                em['adp'].round(4), em['curve'].mean().round(4), em['curve'].max().round(4), fm['adp'].round(4),
...
if (__name__ == '__main__'):
    ...
    for data_name in ['CAMO', 'CHAMELEON', 'COD10K', 'NC4K']:
        ...
        evaluator(snap_path=opt.snap_path, val_root=(('./dataset/TestDataset/' + data_name) + '/'), gpu_id=opt.gpu_id,
                  trainsize=352, if_save_map=opt.if_save_map)
